# Домашнее задание к занятию "`Резервное копирование`" - `Гурылев А.В.`


[Руководство по оформлению Markdown файлов](https://gist.github.com/Jekins/2bf2d0638163f1294637#Code)

---

### Задание 1

-a Архивный режим — сохраняет атрибуты, права, временные метки и рекурсивно копирует всё
-v Выводит подробную информацию о процессе
-h Человеко-понятный вывод размеров файлов
-c Принудительно сравнивает файлы по контрольным суммам (хэшам)
--delete Удаляет файлы из приёмника, которых нет в источнике (зеркалирование)
--exclude='.*/' Исключает все скрытые папки


![123](https://github.com/A1ex93/rsync_homework/blob/main/image/1.png)
![123](https://github.com/A1ex93/rsync_homework/blob/main/image/2.png)


### Задание 2

![123](https://github.com/A1ex93/rsync_homework/blob/main/image/backup_script.png)
![123](https://github.com/A1ex93/rsync_homework/blob/main/image/crontab.png)
![123](https://github.com/A1ex93/rsync_homework/blob/main/image/result_backup.png)
![123](https://github.com/A1ex93/rsync_homework/blob/main/image/result_log.png)
